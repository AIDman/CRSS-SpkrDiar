...
if [ $stage -le 0 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/segment_clustering_ivector.JOB.log \
    segment-clustering-ivector-constraint --explore-mode=$explore_mode --max-explore-pair-percentage=$max_explore_pair_percentage --merge-constraint=$merge_constraint \
