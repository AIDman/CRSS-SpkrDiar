...
bottom_up_clustering(){
    ...
    diar/segment_clustering_ivector_constraint.sh --nj 1 --apply-cmvn-utterance false --apply-cmvn-sliding false \
       		--max-explore-pair-percentage 0.2 --merge-constraint true \
		--do-consolidate true --max-explore-pair-percentage 0.2 \
		--ivector-dist-stop 5.7 exp/ref/$eval_data/segments exp/extractor_256 data/$eval_data exp/clustering_ivector_constraint/$eval_data
