...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    BaseFloat transition_penalty = 0.0;
    ...
    po.Register("transition-penalty", &transition_penalty, "Add some penalty probability to transitions. This is to avoid aburpt changes in vad decisions");
    ...
    for (;!first_reader.Done(); first_reader.Next()) {
      ...
      int32 window = 30;
      // Smooting Likelihood Over Window
      std::vector<Vector<BaseFloat> > smoothed_likes = likes;
      for (int32 i = 0; i < num_classes; i++) {
        for(int32 j = 0; j < like.Dim(); j++) {
          int32 count_num  = 0;
          BaseFloat smoothed_prob = 0;
          if(window < 1) continue;        
          for(int32 k = j - window; k < j + window; k++) {
             if(k >=0 && k < like.Dim()) {
               count_num++;
               smoothed_prob += exp(likes[i](k)); 
             }
             smoothed_likes[i](j) = log(smoothed_prob/count_num);
          }
        }
      }

      Vector<BaseFloat> vad_result(like_dim);
      int32 prev_max_ind = 0;
      for (int32 i = 0; i < like.Dim(); i++) {
        int32 max_indx = 0;
        BaseFloat max_post;
        if (prev_max_ind == 0) {
           max_post = smoothed_likes[0](i) + priors[0] + log(1 - transition_penalty);
        }else {
           max_post = smoothed_likes[0](i) + priors[0] + log(transition_penalty);
        }

        for (int32 j = 0; j < num_classes; j++) {
          BaseFloat other_post;
          if (j == prev_max_ind) {
            other_post = smoothed_likes[j](i) + priors[j] + log(1 - transition_penalty);
          } else {
            other_post = smoothed_likes[j](i) + priors[j] + log(transition_penalty);
          }
