...
bottom_up_clustering(){
    ...
    diar/segment_clustering.sh --nj 1 --lambda 5 exp/ref/$data/segments data/$data exp/clustering/$data	
    #diar/segment_clustering_ivector.sh --nj 1 --apply-cmvn-utterance false --apply-cmvn-sliding false \
    #	--ivector-dist-stop 0.7 exp/clustering/$data/segments exp/extractor_256 data/$data exp/clustering_ivector/$data	
...
ilp_clustering(){
    ...
    diar/construct_ilp_problem.sh --nj 1 --delta 0.5 --apply-cmvn-utterance false --apply-cmvn-sliding false \
	exp/ref/$data/segments exp/extractor_256 data/$data exp/clustering_ilp/$data	

    diar/ilp_clustering.sh exp/clustering/$data/segments exp/clustering_ilp/$data	
