...
mkdir -p $segdir || exit 1;
mkdir -p $logdir || exit 1;
rm -f $segdir/segments.scp
...
$cmd JOB=1:$nj $logdir/vad_segmentation_${name}.JOB.log \
  vad-segmentation --min-silence-len=$min_silence_len --config=$vad_config scp:$sdata/JOB/feats.scp $segdir \
  || exit 1;

nc=`cat $segdir/segments.scp | wc -l` 
...
echo "Created VAD based segmentation output for $name"
