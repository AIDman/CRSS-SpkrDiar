...
for f in $lang/phones/silence.int $lang/phones/nonsilence.int $ali_dir/final.ali.gz $ali_dir/final.mdl; do
...
if [ $stage -le 0 ]; then
  $cmd JOB=1:$num_jobs $dir/log/convert_ali_to_vad.JOB.log \
    ali-to-phones --per-frame=true $ali_dir/final.mdl "ark:gunzip -c $ali_dir/final.ali.gz |" \
