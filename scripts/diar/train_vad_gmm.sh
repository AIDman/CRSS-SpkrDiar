...
if [ $stage -le 0 ]; then
  ...
  if ! $select_top_frames; then
    $cmd $dir/log/init_gmm_speech.log \
      ...
      "$feats select-voiced-frames-v2 ark:- scp:$data/vad.scp ark:- |" \
      $dir/speech.0.mdl || exit 1
    $cmd $dir/log/init_gmm_silence.log \
      gmm-global-init-from-feats --num-gauss=$sil_num_gauss --num-iters=6 \
      "$feats select-voiced-frames-v2 --select-unvoiced-frames=true ark:- scp:$data/vad.scp ark:- |" \
      $dir/silence.0.mdl || exit 1
  else
    $cmd $dir/log/init_gmm_speech.log \
      gmm-global-init-from-feats --num-gauss=$speech_num_gauss --num-iters=12 \
      "$feats select-top-frames --frames-proportion=$top_frames_threshold ark:- ark:- |" \
      $dir/speech.0.mdl || exit 1
    $cmd $dir/log/init_gmm_silence.log \
      gmm-global-init-from-feats --num-gauss=$sil_num_gauss --num-iters=8 \
      "$feats select-top-frames --select-bottom-frames=true --frames-proportion=$bottom_frames_threshold  ark:- ark:- |" \
      $dir/silence.0.mdl || exit 1
  fi
  ...
  $cmd $dir/log/decode.final.log \
    gmm-decode-simple \
    --allow-partial=true --word-symbol-table=$dir/graph_test/words.txt \
